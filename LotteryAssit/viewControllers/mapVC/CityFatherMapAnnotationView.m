...
@implementation CityFatherMapView
...
- (void)adjustMapRegionIfNeeded {
    ...
    CGFloat parentXPosition = [self relativeParentXPosition];
	if (parentXPosition < 38) {
		xPixelShift = 38 - parentXPosition;
	} else if (parentXPosition > self.frame.size.width - 38) {
		xPixelShift = (self.frame.size.width - 38) - parentXPosition;
	}
...
- (void) preventParentSelectionChange {
    objc_setAssociatedObject(self.parentAnnotationView,@"preFlag",[NSNumber numberWithBool:YES],OBJC_ASSOCIATION_RETAIN);
}

- (void) allowParentSelectionChange {
    [self.mapView selectAnnotation:self.parentAnnotationView.annotation animated:NO];
    
    objc_setAssociatedObject(self.parentAnnotationView,@"preFlag",[NSNumber numberWithBool:NO],OBJC_ASSOCIATION_RETAIN);
}
